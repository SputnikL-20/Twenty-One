...
//		}
		...
		labelScore = new JLabel("Test Label Score");
		labelScore.setSize(125, 22);
...
//				} while (resultpc < 19);
		...
		btnYou.setCursor(new Cursor(Cursor.HAND_CURSOR));
...
//							.addComponent(btnNewGame, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
							.addComponent(btnNewGame, GroupLayout.PREFERRED_SIZE, 22, GroupLayout.PREFERRED_SIZE)
							.addGap(17))
	...
	private void initializeDeck() {
		...
		String[][] tmpdeck = testDeck(shufflingTheDeck(fullDeck()));
 		...
 		for (int i = 0; i < m; i++) {
 			for (int j = 0; j < n; j++) {
				System.out.print("tmpdeck["+i+"]["+j+"] = ["+ tmpdeck[i][j]+"] "); //  i + " = [" + tmpdeck[i][j] + "]");
			}
			System.out.println();
		}
...
//		TestAppPoker window = new TestAppPoker();
				...
				try {
					for (int i = count; i < count + 1; i++) {
						if (resultpc < 19) {
							...
							label_[i].setVisible(true);
							count++; // счётчик взятых карт
							LabelTotal.setText(String.valueOf(resultpc));  
							Thread.sleep(500);
						}
					}
					writeResultPc();
	...
	public static void main(String[] args) {
		try {
			TestAppPoker testStart = new TestAppPoker();
			
			testStart.gameVisible();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
